...
final class Client extends Thread {
    ...
    static final int CLIENTMASK = (((1 << BITSFORCLIENTS) - 1) << BITSFORRESOURCES);
        ...
        try {
            while (true) {
                ...
                case 112:
                    // SetCloseDownMode
                    length--;
                    if (data == AllTemporary || data == RetainPermanent
                ...
                if (errorReason != 0) {
                    ...
                    cevent.mkError(errorReason, errorValue, (reqType <= 127 ? 0
                            : data), reqType);
                    sendEvent(cevent, 1, 0, Event.NoEventMask, null);
                }
                client.flush();
            }
        } catch (Exception e) {
    ...
    private static final void reqGrabServer(Client c) throws IOException {
        ...
        while (true) {
            ...
            for (i = 1; i < clients.length; i++) {
                if (i == c.index || clients[i] == null || !clients[i].isAlive()
                        || clients[i].waitforreq || clients[i].suspended)
                    continue;
    ...
    static final void closeDownRetainedResources() {
        synchronized (LOCK) {
            for (int i = 1; i < clients.length; i++) {
                Client c = clients[i];
                if (c != null && (c.closeDownMode == RetainTemporary)
                        && (c.clientGone)) {
                    c.closeDown();
                }
            }
    ...
    final int sendEvent(Event event, int count, int mask, int filter, Grab grab)
